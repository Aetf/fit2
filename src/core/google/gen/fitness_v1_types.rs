...
use async_google_apis_common::{
    ...
    EmptyRequest, do_request, DeserializeOwned,
    Result, hyper,
};
pub use async_google_apis_common::{TlsClient, TlsConnr, ApiError};

use crate::auth::Authenticator;
use hyper::StatusCode;
use std::fmt;

pub fn default_https_client() -> TlsClient {
    let conn = hyper_rustls::HttpsConnector::new();
    let cl = hyper::Client::builder().build(conn);
    cl
}

async fn do_request_auth<
    Req: Serialize + std::fmt::Debug + Clone,
    Auth: Authenticator,
    Resp: DeserializeOwned + Clone + Default,
    SIntoIter: IntoIterator<IntoIter=SIter> + Send,
...
{
    let tok = auth.token(scopes).await?;
    do_request(
        cl,
        path,
        &[
            (hyper::header::AUTHORIZATION, format!("Bearer {}", tok.secret()))
        ],
        http_method,
        rq
    ).await
...
async fn do_request_retry<
    Req: Serialize + std::fmt::Debug + Clone,
    Auth: Authenticator,
    Resp: DeserializeOwned + Clone + Default,
    SIntoIter: IntoIterator<IntoIter=SIter> + Clone + Send,
    SIter: Iterator<Item=S> + Send,
    S: ToString,
>(
    cl: &TlsClient,
    ...
    match do_request_auth(cl, path, http_method, rq.clone(), scopes.clone(), auth).await {
        Ok(resp) => Ok(resp),
        Err(e) => {
            match e.downcast_ref() {
                Some(
                    ApiError::HTTPResponseError(status, ..)
                ) if matches!(*status, StatusCode::FORBIDDEN | StatusCode::UNAUTHORIZED) => {
                    // try refresh
                    auth.refresh(scopes.clone()).await?;
                    do_request_auth(cl, path, http_method, rq, scopes, auth).await
                },
                // propagate error
                _ => Err(e),
            }
...
impl fmt::Display for FitnessScopes {
...
pub struct UsersDataSourcesDatasetsPatchParams {
    ...
    #[serde(rename = "datasetId")]
    pub dataset_id: String,
    /// i64: The client's current time in milliseconds since epoch. Note that the minStartTimeNs and maxEndTimeNs properties in the request body are in nanoseconds instead of milliseconds.
...
impl std::fmt::Display for UsersDataSourcesDatasetsPatchParams {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(ref v) = self.current_time_millis {
            write!(
                f,
                "&currentTimeMillis={}",
                percent_encode(format!("{}", v).as_bytes(), NON_ALPHANUMERIC).to_string()
...
pub struct UsersDataSourcesDatasetsDeleteParams {
    ...
    #[serde(rename = "userId")]
    pub user_id: String,
    /// i64: The client's current time in milliseconds since epoch.
    #[serde(rename = "currentTimeMillis")]
    pub current_time_millis: Option<String>,
    /// Dataset identifier that is a composite of the minimum data point start time and maximum data point end time represented as nanoseconds from the epoch. The ID is formatted like: "startTime-endTime" where startTime and endTime are 64 bit integers.
    #[serde(rename = "datasetId")]
    pub dataset_id: String,
    /// i64: When the operation was performed on the client.
    #[serde(rename = "modifiedTimeMillis")]
    pub modified_time_millis: Option<String>,
}
...
impl std::fmt::Display for UsersDataSourcesDatasetsDeleteParams {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(ref v) = self.modified_time_millis {
            write!(
                f,
                "&modifiedTimeMillis={}",
                percent_encode(format!("{}", v).as_bytes(), NON_ALPHANUMERIC).to_string()
...
pub struct UsersDataSourcesService<A> {
    client: TlsClient,
    authenticator: A,
    scopes: Vec<String>,
...
impl<A: Authenticator> UsersDataSourcesService<A> {
    ...
    pub fn new(client: TlsClient, auth: A) -> UsersDataSourcesService<A> {
    ...
    pub async fn delete(&mut self, params: &UsersDataSourcesDeleteParams) -> Result<DataSource> {
        let rel_path = format!(
            "{userId}/dataSources/{dataSourceId}",
            userId = percent_encode(params.user_id.as_bytes(), NON_ALPHANUMERIC),
            dataSourceId = percent_encode(params.data_source_id.as_bytes(), NON_ALPHANUMERIC)
        );
    ...
    pub async fn list(
        ...
        headers.push((
            ...
            format!("Bearer {token}", token = tok.secret()),
        ));
    ...
    pub async fn get(&mut self, params: &UsersDataSourcesGetParams) -> Result<DataSource> {
        let rel_path = format!(
            "{userId}/dataSources/{dataSourceId}",
            userId = percent_encode(params.user_id.as_bytes(), NON_ALPHANUMERIC),
            dataSourceId = percent_encode(params.data_source_id.as_bytes(), NON_ALPHANUMERIC)
...
pub struct UsersDataSourcesDataPointChangesService<A> {
    client: TlsClient,
    authenticator: A,
    scopes: Vec<String>,
...
impl<A: Authenticator> UsersDataSourcesDataPointChangesService<A> {
    /// Create a new UsersDataSourcesDataPointChangesService object. The easiest way to call this is wrapping the Authenticator
    /// into an `Rc`: `new(client.clone(), Rc::new(authenticator))`.
    /// This way, one authenticator can be shared among several services.
    pub fn new(client: TlsClient, auth: A) -> UsersDataSourcesDataPointChangesService<A> {
...
pub struct UsersDataSourcesDatasetsService<A> {
    client: TlsClient,
    authenticator: A,
    scopes: Vec<String>,
...
impl<A: Authenticator> UsersDataSourcesDatasetsService<A> {
    /// Create a new UsersDataSourcesDatasetsService object. The easiest way to call this is wrapping the Authenticator
    /// into an `Rc`: `new(client.clone(), Rc::new(authenticator))`.
    /// This way, one authenticator can be shared among several services.
    pub fn new(client: TlsClient, auth: A) -> UsersDataSourcesDatasetsService<A> {
    ...
    pub async fn get(&mut self, params: &UsersDataSourcesDatasetsGetParams) -> Result<Dataset> {
        let rel_path = format!(
            "{userId}/dataSources/{dataSourceId}/datasets/{datasetId}",
            userId = percent_encode(params.user_id.as_bytes(), NON_ALPHANUMERIC),
            dataSourceId = percent_encode(params.data_source_id.as_bytes(), NON_ALPHANUMERIC),
            datasetId = percent_encode(params.dataset_id.as_bytes(), NON_ALPHANUMERIC)
        );
        let path = self.format_path(rel_path.as_str());
    ...
    pub async fn delete(&mut self, params: &UsersDataSourcesDatasetsDeleteParams) -> Result<()> {
        let rel_path = format!(
            "{userId}/dataSources/{dataSourceId}/datasets/{datasetId}",
            userId = percent_encode(params.user_id.as_bytes(), NON_ALPHANUMERIC),
            dataSourceId = percent_encode(params.data_source_id.as_bytes(), NON_ALPHANUMERIC),
...
pub struct UsersSessionsService<A> {
    client: TlsClient,
    authenticator: A,
    scopes: Vec<String>,
...
impl<A: Authenticator> UsersSessionsService<A> {
    /// Create a new UsersSessionsService object. The easiest way to call this is wrapping the Authenticator
    /// into an `Rc`: `new(client.clone(), Rc::new(authenticator))`.
    /// This way, one authenticator can be shared among several services.
    pub fn new(client: TlsClient, auth: A) -> UsersSessionsService<A> {
    ...
    pub async fn update(
        ...
        let rel_path = format!(
            ...
            userId = percent_encode(params.user_id.as_bytes(), NON_ALPHANUMERIC),
            sessionId = percent_encode(params.session_id.as_bytes(), NON_ALPHANUMERIC)
        );
        let path = self.format_path(rel_path.as_str());
    ...
    pub async fn delete(&mut self, params: &UsersSessionsDeleteParams) -> Result<()> {
        let rel_path = format!(
            "{userId}/sessions/{sessionId}",
            userId = percent_encode(params.user_id.as_bytes(), NON_ALPHANUMERIC),
            sessionId = percent_encode(params.session_id.as_bytes(), NON_ALPHANUMERIC)
    ...
    pub async fn list(&mut self, params: &UsersSessionsListParams) -> Result<ListSessionsResponse> {
        let rel_path = format!(
            "{userId}/sessions",
            userId = percent_encode(params.user_id.as_bytes(), NON_ALPHANUMERIC)
        );
...
pub struct UsersDatasetService<A> {
    client: TlsClient,
    authenticator: A,
    scopes: Vec<String>,
...
impl<A: Authenticator> UsersDatasetService<A> {
    /// Create a new UsersDatasetService object. The easiest way to call this is wrapping the Authenticator
    /// into an `Rc`: `new(client.clone(), Rc::new(authenticator))`.
    /// This way, one authenticator can be shared among several services.
    pub fn new(client: TlsClient, auth: A) -> UsersDatasetService<A> {
        UsersDatasetService {
            client,
            authenticator: auth,
            scopes: vec![],
